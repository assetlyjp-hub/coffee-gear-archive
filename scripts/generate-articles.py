...
引数:
  --type  : 記事カテゴリ (review/method/recipe/comparison/guide/beginner)
...
def generate_article(client: anthropic.Anthropic, keyword: dict, lang: str,
                     gear_data: list, brewmethods_data: list, brands_data: list) -> str:
    """
    Claude APIを呼び出して記事を生成する。
    器具・抽出方法・ブランドデータをコンテキストとして渡す。
    """
    # 言語に応じたプロンプト
    if lang == "ja":
        lang_instruction = """
記事は日本語で書いてください。
ターゲット読者は日本のコーヒー愛好家です。
"""
    else:
        lang_instruction = """
Write the article in English.
Target audience is home baristas and coffee enthusiasts interested in brewing better coffee.
"""

    # システムプロンプト
    system_prompt = f"""You are an expert coffee gear reviewer and brewing specialist for Coffee Gear Archive (coffeegeararchive.com).
You write authoritative, detailed, and engaging content about coffee brewing equipment and methods.
Your tone is knowledgeable but accessible — like a skilled barista explaining to an eager student.
{lang_instruction}
...
GEAR DATA (first 10):
{json.dumps(gear_data[:10], indent=2, ensure_ascii=False)[:3000]}

BREW METHODS DATA:
{json.dumps(brewmethods_data, indent=2, ensure_ascii=False)[:2000]}
...
2. Article body:
   - 800-1500 words
   - Use ## for H2, ### for H3
   - Include specific gear recommendations with prices
   - Reference actual products from the data
...
def main():
    # コマンドライン引数のパース
    parser = argparse.ArgumentParser(description="Coffee Gear Archive 記事自動生成スクリプト")
    parser.add_argument("--type", type=str, default="guide",
                        choices=["review", "method", "recipe", "comparison", "guide", "beginner"],
    ...
    gear_path = project_root / "src" / "data" / "gear.json"
    brewmethods_path = project_root / "src" / "data" / "brewmethods.json"
    brands_path = project_root / "src" / "data" / "brands.json"

    # データの読み込み
    gear_data = load_json(str(gear_path))
    brewmethods_data = load_json(str(brewmethods_path))
    ...
    print(f"=== Coffee Gear Archive 記事生成 ===")
    ...
    for i, keyword in enumerate(keywords_to_generate, 1):
        ...
        try:
            ...
            content = generate_article(
                ...
                gear_data, brewmethods_data, brands_data
            )
